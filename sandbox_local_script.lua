...
local scrollingFrame = frame:FindFirstChild("ScrollingFrame") or gui:FindFirstChild("ScrollingFrame", true)
...
local function setActiveCatalog(showGeneral)
	local hasGeneralCatalog = scrollingFrame and scrollingFrame:IsA("GuiObject")
	if hasGeneralCatalog then
	...
	if scrollingDropper and scrollingDropper:IsA("GuiObject") then
		if hasGeneralCatalog then
			...
			-- Si no existe catálogo general, dejamos visible el de droppers.
			scrollingDropper.Visible = true
		end
	end
end
